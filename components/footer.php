<?php
//Footer content
?>
<footer>
    <div class="footer-logo">
        <img src="img/logo-black.png" alt="Kalibar" class="img-responsive"/>
        <br>
        <i class="fa fa-copyright" aria-hidden="true"></i><p class="footer-text">All rights reserved Kalibar</p>
        <br>
        <div class="social-icons-footer">
            <a href="https://www.instagram.com/kalibaraarhus/?hl=en"><i class="fa fa-instagram" aria-hidden="true"></i></a>
            <a href="https://www.facebook.com/KALIBARAARHUS/"><i class="fa fa-facebook" aria-hidden="true"></i></a>   
        </div>
    </div>
</footer>
